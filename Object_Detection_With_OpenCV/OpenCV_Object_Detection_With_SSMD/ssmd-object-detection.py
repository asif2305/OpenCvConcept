...
if __name__ == "__main__":
    ...
    model ="SSMD\MobileNetSSD_deploy.prototxt"
    weights = "SSMD\MobileNetSSD_deploy.caffemodel"
    labelFile = 'label.txt'
    openCVSSMD =  OpenCvSSMD(model, weights,labelFile)
    # vedio image load
    ...
    while (cap.isOpened()):
        ...
        boxes, masks = openCVSSMD.MobileNetSSD_network(image)
         # for box in boxes:
        detection_count = boxes.shape[2]
        img =  openCVSSMD.detectionImage(image, detection_count, boxes)
        cv2.imshow("img", img)
